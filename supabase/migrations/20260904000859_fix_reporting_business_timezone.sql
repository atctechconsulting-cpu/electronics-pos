...
begin
  ...
  /*
 * Convert the requested UK business dates into absolute timestamp
 * boundaries once and reuse them throughout the report.
 *
 * The interval is half-open:
 *   [start of first day, start of day after last day)
 *
 * Europe/London is intentional so reporting follows the retailer's
 * local trading day across both GMT and BST.
 */
v_period_start :=
...
v_period_end :=
  ...
  where r.organization_id = v_organization_id
    ...
    and coalesce(r.completed_at, r.created_at) >= v_period_start
    and coalesce(r.completed_at, r.created_at) < v_period_end
    and (
      p_branch_id is null
  ...
  daily_sales as (
    select
      (s.completed_at at time zone 'Europe/London')::date as report_date,
    ...
    group by (s.completed_at at time zone 'Europe/London')::date
  ),
  daily_returns as (
    select
      (
