...
begin
  ...
  if v_user_id is null then
    raise exception 'You must be signed in to complete a return.';
  end if;
  ...
  if not found then
    raise exception 'The selected sale does not exist.';
  end if;
  ...
  if not public.is_branch_member(v_sale.branch_id) then
  ...
  loop
    begin
      ...
      v_serial_ids := coalesce(v_item -> 'serial_ids', '[]'::jsonb);
    exception
      when others then
        raise exception 'A return item contains invalid data.';
    end;
    ...
    if not found then
      raise exception 'A selected sale item does not belong to this sale.';
    end if;
    ...
    v_line_refund :=
      round(v_sale_item.unit_price * v_quantity, 2);
    ...
    if jsonb_typeof(v_serial_ids) <> 'array' then
      raise exception 'Serial IDs must be provided as an array.';
    end if;
    ...
    if v_serial_count > 0 then
      ...
      for v_serial_id in
        select value::uuid
        from jsonb_array_elements_text(v_serial_ids)
      loop
        if not exists (
          select 1
  ...
  v_refund_total := round(v_refund_total, 2);
